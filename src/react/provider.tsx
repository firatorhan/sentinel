...
export const SentinelProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [dialogMeta, setDialogMeta] = useState<{ title?: string; description?: string }>({});
  ...
  const unregisterHover = (id: string) => {
    ...
    setActiveRect((prev) => (prev?.width && activeId === id ? null : activeRect));
  };
  ...
  return (
    ...
    >
      ...
      <Portal>
        ...
        <Dialog open={openDialogId !== null} onOpenChange={(open) => !open && closeDialog()}>
          <DialogContent onClick={(e) => e.stopPropagation()}>
            <DialogHeader>
              <DialogTitle>{dialogMeta.title || "Edit profile"}</DialogTitle>
              <DialogDescription>{dialogMeta.description}</DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <Button type="button" onClick={closeDialog}>Save changes</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </Portal>
