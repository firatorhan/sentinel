...
- `useSentinelInteraction()` — returns `{ activeId, activeRect, isActive, setIsActive, showOutlines, setShowOutlines, highlightName, setHighlightName, reduxStore, registerHover, unregisterHover, openDialog }`. Returns noops outside a provider (same for `useSentinelDialog`).
