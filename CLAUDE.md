...
- `Sentinel` — wrapper component injected by the plugin. Registers hover/click handlers and tracks render count + prop history (last 6 snapshots via `propHistoryRef`). Passes `DOMRect` (not element) to `registerHover`. Props are serialized via `safeSerialize` (strips React internals, circular refs, functions) before storage. On every render (independent of `isActive`) it also upserts its current props into the provider's ambient `componentRegistry` keyed by instance id, and unregisters on unmount — this is what feeds the MCP `get_component_props` tool (click only gates the dialog, not registry collection).
- `createSentinelSagaMonitor(options?)` — factory for a Redux Saga monitor object. Pass the result to `SentinelProvider sagaMonitor=` and to the saga middleware as `sagaMiddleware.run(rootSaga, { sagaMonitor })`. Supports both redux-saga v1.x (`CALL.payload`) and v0.x (`CALL` direct). Exposes `_getEffects()` (raw), `_getSerializableEffects()` (SSR-safe, strips Axios internals/circular refs), `_subscribe()`, `_clear()`. Options: `maxRecords?` (default 100).
- `createSentinelReduxMiddleware(options?)` — factory for a Redux middleware that records dispatched actions with top-level state diffs. Exposes `middleware`, `_getRecords()`, `_getSerializableRecords()`, `_subscribe()`, `_clear()`. Options: `maxRecords?` (default 50).
- `useSentinelInteraction()` — returns `{ activeId, activeRect, isActive, setIsActive, showOutlines, setShowOutlines, highlightName, setHighlightName, reduxStore, componentRegistry, registerHover, unregisterHover, openDialog }`. Returns noops outside a provider (same for `useSentinelDialog`); `componentRegistry` falls back to `noopComponentRegistry`.
...
- `ui/widgets/` — composite widgets:
  ...
  - `SentinelDialog` — tabbed dialog with three tabs: `.md` (markdown doc), `Props Tracker` (current props + history via `PropsViewer`), `API Layer` (via `ApiLayerViewer`). Header shows `vscode://file/...` link, render count badge, and resolved `externalLinks` buttons.
  - `ApiLayerViewer` — extracts HTTP calls from saga effects (`utils/apiCalls.ts`: handles raw client Axios objects and serialized server effects), correlates component props with response data path-by-path (props-match filter + "props mapping" accordion), highlights matched fields in the response `JsonNode` tree, and offers copy-as-cURL. Calls are tagged `client`/`server` (origin badge shown when both exist).
  - `SentinelToolbar` — floating `ScanEye` button (bottom-right, `z-[9999]`) rendered via Portal. Opens a Popover with four tabs: **Controls** (Active toggle, Outline All toggle, component name filter), **State** (live Redux state tree with search + accordion), **Log** (action log with state diffs; framework actions `@@…`/`persist/…` hidden behind a System toggle, consecutive duplicate actions grouped with ×N badge), **Saga** (effect tree with deep search; type filter defaults to CALL-only). Tabs with a server-side data source (`serverState`, `serverActionLog`, `serverSagaEffects`) get a Client/Server toggle via the shared `ClientServerSection`; all three panes share the `ExpandablePane` shell (search + actions + expand-to-dialog). Keyboard shortcut `Ctrl+Shift+S` toggles `isActive`.
